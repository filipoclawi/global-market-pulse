...
try:
 ...
 with sync_playwright() as p:
  ...
  stale=browser.new_page();stale.route('**/data/market-data.json*',lambda route:route.fulfill(status=200,content_type='application/json',body=json.dumps(stale_payload)));stale.goto('http://127.0.0.1:8765',wait_until='networkidle');stale.wait_for_function("document.documentElement.dataset.ready === 'true'");assert stale.locator('#stale-warning').is_visible();assert stale.locator('#stale-warning').inner_text()=='1 source stale';assert stale.locator('#update-status').get_attribute('class').endswith('has-stale');stale.close();browser.close()
