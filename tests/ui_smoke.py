...
try:
 ...
 with sync_playwright() as p:
  ...
  first=page.locator('.news-story').first;first.locator('.story-toggle').click();assert first.locator('.story-body').is_visible();assert first.locator('.story-toggle').get_attribute('aria-expanded')=='true';assert page.locator('#tab-unread-count').inner_text()=='4'
  first.locator('.star-toggle').click();assert page.locator('.news-story').first.locator('.star-toggle').get_attribute('aria-pressed')=='true'
  ...
  archived_page=browser.new_page();archived_page.add_init_script("localStorage.setItem('global-market-pulse:news-state:v1', JSON.stringify({readIds:['archived-star-test'],starredIds:['archived-star-test']}))");archived_page.route('**/data/economic-news.json*',lambda route:route.fulfill(status=200,content_type='application/json',body=json.dumps(archive_payload)));archived_page.goto('http://127.0.0.1:8765',wait_until='networkidle');archived_page.wait_for_function("document.documentElement.dataset.newsReady === 'true'");assert archived_page.locator('#filter-starred-count').inner_text()=='1';archived_page.get_by_role('button',name='Starred 1').click();assert archived_page.locator('[data-story-id="archived-star-test"]').count()==1;archived_page.close()
